export default class VariableDeclaration {
  ...
  generate(block) {
    ...
    var code = `var ${varName} = ${t.initializer(block)}; /* has type ${
      t.generate(block)
    } */`;
    ...
    if (theType.name == "alphafile") {
      code = `var ${varName} = [];\n`;
      code = code +
        `for( var ${varName}_i = 0; ${varName}_i <= ${range}; ${varName}_i++ )\n`;
    ...
    if (theType.name == "memoryword") {
      ...
      code = code +
        `var ${varName}_gr = new Float32Array(${varName}.buffer);\n`;
      ...
      for (var i in t.fields) {
        ...
        for (var j in t.fields[i].names) {
          ...
          if (fieldType.fields == undefined) {
            code = code +
              `var ${varName}_${n.generate(block)} = new ${
                fieldType.intish(block)
              }Array(${range});`;
          } else {
            if (f.type.name == "memoryword") {
              ...
              code = code +
                `var ${varName}_${
                  n.generate(block)
                }_int = new Int32Array(${varName}_${
                  n.generate(block)
                }.buffer);\n`;
              code = code +
                `var ${varName}_${
                  n.generate(block)
                }_gr = new Float32Array(${varName}_${
                  n.generate(block)
                }.buffer);\n`;
              code = code +
                `var ${varName}_${
                  n.generate(block)
                }_hh = new Int16Array(${varName}_${
                  n.generate(block)
                }.buffer);\n`;
              code = code +
                `var ${varName}_${
                  n.generate(block)
                }_qqqq = new Int8Array(${varName}_${
                  n.generate(block)
                }.buffer);`;
            } else {

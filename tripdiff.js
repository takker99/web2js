...
import { diffLines } from "npm:diff";
import colors from "npm:colors";

const testeeFilename = process.argv[2];
const originalFilename = process.argv[3];

const testee = readFileSync(testeeFilename).toString();
const original = readFileSync(originalFilename).toString();

// I am not using ./ in filenames
original = original.replace("(./etrip.tex", "(etrip.tex");
original = original.replace(/\(\.\/etrip\.out/g, "(etrip.out");

let diffs = diffLines(testee, original);
...
function isNotTooDifferent(a, b) {
  ...
  if (
    b == " 7i,7n,9p,113b,38s stack positions out of 200i,40n,60p,500b,600s\n"
  ) {
  ...
  if (
    (b == "Hyphenation trie of length 434 has 12 ops out of 35111\n") &&
  ...
  if (b.match(/, glue set /)) {
    const lefts = a.split(", glue set ");
    const rights = b.split(", glue set ");

    if (lefts[0] == rights[0]) {
      const l = parseFloat(lefts[1].replace("fil", ""));
      const r = parseFloat(rights[1].replace("fil", ""));
  ...
  if (
    a.startsWith("Memory usage before: ") &&
    b.startsWith("Memory usage before: ")
  ) {
...
for (const diff of diffs) {
  ...
  const result = isNotTooDifferent(diff[0].value, diff[1].value);
